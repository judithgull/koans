...
import { SeriesQuerySuccess, AppState, reducers } from '../index';
...
describe('Series Reducer and Selectors', () => {
  ...
  describe('series by authorId', () => {
    ...
    it('returns series with same author', done => {
      const series$ = store.select(
        SeriesQueries.byAuthorId(mockSeries[0].authorId)
      );
      store.dispatch(new SeriesQuerySuccess(mockSeries));
  ...
  describe('select', () => {
    ...
    it('does not select anything, if exercise is not found', done => {
      ...
      store.dispatch(new ExerciseSelectAction(exerciseNr));
      subs.push(
        selectedExercise$.subscribe(value => {
          expect(value).toBeUndefined();
          done();
        })
      );
    ...
    it('deselects a series', done => {
      const selectedId$ = store.select(SeriesQueries.selectedSeriesId);

      const seriesId = 'a';
      store.dispatch(new SeriesSelectAction(seriesId));
      store.dispatch(new SeriesDeselectAction());

      subs.push(
        selectedId$.pipe(filter(v => !v)).subscribe(value => {
    ...
    it('deselects an exercise', done => {
      ...
      subs.push(
        selectedExercise$.pipe(filter(v => !v)).subscribe(value => {
          expect(value).toBeFalsy();
          done();
        })
      );
